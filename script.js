...
const contactSchema = z.object({
  ...
  // Honeypot field. A real visitor never sees or fills this (see the
  // .hp-field CSS), so z.literal("") demands it stay empty. If a bot
  // fills every input it can find, this fails validation and we quietly
  // drop the submission in section 6 below.
  company: z.literal("", {
...
form.addEventListener("submit", async (event) => {
  // Stop the browser's default "reload the page and send a plain HTTP
  // request" behavior — we want to send it ourselves with fetch() so we
  // can validate first and show inline feedback without a page reload.
  event.preventDefault();

  clearFieldErrors();
  setStatus("", "");

  // FormData reads every named input/textarea inside the <form> into a
  // key/value collection, keyed by each field's `name` attribute.
  const formData = new FormData(form);
  const values = Object.fromEntries(formData.entries());

  // safeParse (rather than parse) returns a result object instead of
  // throwing, which keeps this code readable as a plain if/else instead
  // of a try/catch.
  ...
  if (!result.success) {
    ...
    // Honeypot tripped: a bot filled the hidden "company" field. We do
    // NOT reveal that to the caller — no special error message, no
    // console log — we just pretend the message was sent and stop here.
    // This avoids teaching spam scripts that they've been detected.
  ...
  if (!accessKey || accessKey === "PASTE_YOUR_WEB3FORMS_ACCESS_KEY_HERE") {
    // Fails loudly in development so it's obvious the key still needs
    // to be added — see the data-web3forms-key attribute on the <form>
    // in index.html.
    setStatus(
  ...
  try {
    ...
    if (payload.success) {
      ...
      // disabled or invalid access key) — payload.message explains why.
      setStatus(payload.message || "Something went wrong. Please try again.", "error");
    }
  } catch (error) {
    // Network failure, offline, DNS issue, etc. — fetch() throws rather
    // than resolving in this case.
